...
One important concept in LINQ is **Deferred Execution.** LINQ queries are not executed 
when they are defined but when they are iterated over (e.g., using a foreach loop). 
...
```
**Immediate Execution** occurs when methods like ToList(), ToArray(), First(), or Count() are called,
which forces the query to execute immediately.
```
...
```
**Group Join:** Combines data and groups the results.
```
